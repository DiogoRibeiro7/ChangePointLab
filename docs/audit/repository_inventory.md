...
- BOCPD: `BOCPD`, `BOCPDConfig`, `BOCPDAlertConfig`, `BOCPDResult`, `BetaBernoulli`, `PoissonGamma`, `Hazard`, `ConstantHazard`, `BoostedBoundaryHazard`, `ScheduledHazard`
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PELT | `algorithms/optimization/pelt.py`, `cost_functions.py` | `PELT`, `pelt`, `pelt_detect` | Killick et al. (2012) adaptation; bundled costs now have independent optimal-partitioning oracle tests | 1-D finite numeric sequence plus `SegmentCost`; returns `PELTResult` or `ChangePointResult` | Right-exclusive interior boundaries in `[1, n-1]` | Deterministic | Unit parity, result fields, interoperability, exhaustive oracle tests, examples | Current implementation uses exact candidate retention for bundled costs and does not claim pruning-dependent linear complexity |
| BOCPD | `algorithms/bayesian/bocpd/*` | `BOCPD`, `BOCPDConfig`, `BOCPDAlertConfig`, `BetaBernoulli`, `PoissonGamma`, hazards, plotting helpers, `bocpd-cli` | Adams & MacKay (2007) adaptation with Beta-Bernoulli and Poisson-Gamma oracle tests | Binary or scalar count streams for implemented likelihoods; wrapper returns explicit alert-policy result | Canonical `P(r_t = 0 | x_1:t)` posterior by default; alert indices come from `BOCPDAlertConfig` | Deterministic for current Beta-Bernoulli and Poisson-Gamma paths | Hand-computed posterior, independent recursion, Poisson predictive formula, normalization, alert policy, approximation diagnostics, checkpoint/resume, missing data, edge cases, parity, CLI, docs | Gaussian/Student-t BOCPD remains unsupported; deprecated `cp_scale` compatibility mode is not calibrated |
...
| E-Divisive | `algorithms/nonparametric/edivisive.py`, `edivisive_core.py` | `EDivisive`, `edivisive`, `EDivisiveResult`, `EDivisiveSplit` | Matteson & James (2014) adaptation with independent energy-statistic fixtures | Numeric sequence/matrix; returns recursive split result and labels | Right-exclusive interior split indices; admissible splits include `m - min_size` | Uses owned `numpy.random.Generator` for permutation/bootstrap paths and result provenance; execution is sequential (`n_jobs=1`) | Memmap, parity, interoperability, examples, reproducibility, independent energy-statistic, resampling, null-calibration, and power-curve tests | External R/Python implementation parity and broad multiple-testing calibration remain pending |
| Kernel CPD / exact KCP | `algorithms/kernel/kcp.py`, `kcp_core.py`, `bandwidth_cv.py` | `KernelCPD`, `KernelMatrix`, `gram_rbf`, `kcp_penalized`, `kcp_select_bic` | Harchaoui & Bach plus Arlot/Celisse/Harchaoui registry references | Numeric matrix, typed Gram result, or precomputed prefix depending on layer; returns `KCPResult`/`SegmentationResult` with kernel metadata | Right-exclusive interior split indices | Deterministic for exact path | Segment-bounds, baseline, linear feature-space oracle, independent RBF Gram oracle, Gram-validation, and bandwidth-CV tests | Penalty-selection and large-scale bandwidth-selection validation remain pending |
| RFF KCP | `algorithms/kernel/kcp_rff.py`, `rff_variants.py` | `RFFConfig`, RFF helpers, `KernelCPD(..., rff_config=...)`, and CLI adapters | Rahimi & Recht approximation plus KCP source references | Numeric matrix; approximate RBF kernel segmentation with feature-prefix costs | Right-exclusive interior split indices | Uses explicit local `numpy.random.Generator` streams | Exact/RFF convergence smoke, RFF metadata, package, and performance tests | Approximation quality is validated on small deterministic fixtures only; broad convergence benchmarks remain pending |
...
- Editable install in a clean virtual environment succeeded and imported `changepoint_lab.__version__ == "0.1.14"`.
- `python -m build` succeeded and produced wheel and sdist.
- Wheel installation in a clean virtual environment succeeded and imported `changepoint_lab.__version__ == "0.1.14"`.
...
- Current version in package metadata, runtime, CFF, and Zenodo JSON: `0.1.16`
