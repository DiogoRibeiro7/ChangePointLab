...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PELT | `algorithms/optimization/pelt.py`, `cost_functions.py` | `PELT`, `pelt`, `pelt_detect` | Killick et al. (2012) is the expected primary source; current code needs explicit traceability docs | 1-D numeric sequence plus `SegmentCost`; returns `PELTResult` or `ChangePointResult` | Interior boundary indices in `[1, n-1]`; half-open segment edges implied | Deterministic | Unit parity, result fields, interoperability, examples | Penalty semantics and cost-specific oracle coverage still need proof |
| BOCPD | `algorithms/bayesian/bocpd/*` | `BOCPD`, `BOCPDConfig`, hazards, plotting helpers, `bocpd-cli` | Adams & MacKay (2007) expected; likelihood support is uneven | Binary/count-like stream depending on likelihood; wrapper returns thresholded `ChangePointResult` | Wrapper flags indices where `cp_prob > 0.5` | Deterministic for current Beta-Bernoulli path | Edge cases, parity, CLI, docs | `cp_prob` scaling and hard-coded extraction threshold require calibration audit; PoissonGamma/GaussianNIW contain placeholders |
...
- Editable install in a clean virtual environment succeeded and imported `changepoint_lab.__version__ == "0.1.5"`.
- `python -m build` succeeded and produced wheel and sdist.
- Wheel installation in a clean virtual environment succeeded and imported `changepoint_lab.__version__ == "0.1.5"`.
...
- Current version in package metadata, runtime, CFF, and Zenodo JSON: `0.1.5`
