...
| R-007 | High | Scientific correctness | Within-period prior/proposal details are not independently verified; prior audit identified Poisson lambda and reverse-proposal concerns. | Sampler may not target the stated posterior distribution. | Build paper-derived and brute-force small-state oracles before changing behavior. |
...
| ID | Status | Evidence |
| --- | --- | --- |
| RR-001 | Resolved before this audit | `pyproject.toml`, runtime `__version__`, CFF, and `.zenodo.json` are aligned at `0.1.6`. |
