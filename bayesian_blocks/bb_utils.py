...
def bootstrap_confidence_intervals(
    ...
    edge_ci = np.array([[original_result.edges[0], original_result.edges[-1]]])  # Placeholder
    value_ci = np.column_stack([
        np.percentile([r.block_value for r in bootstrap_results], lower_percentile, axis=0),
        np.percentile([r.block_value for r in bootstrap_results], upper_percentile, axis=0)
    ])
...
def _predict_from_blocks(data: NDArray[np.floating], result: BBResult) -> NDArray[np.floating]:
...
def merge_small_blocks(
    result: BBResult,
    min_size: float,
    merge_criterion: str = "nearest_value"
) -> BBResult:
    """
...
def quick_analysis(data: NDArray[np.floating], 
                  show_plots: bool = True,
                  confidence_intervals: bool = False) -> dict:
    """
